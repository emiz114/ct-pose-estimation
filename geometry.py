...
class Transform:
    ...
    # for applying to an image matrix in 3D or a multichannel image matrix in 3D + 1D
    def rotate_array(self, array, padwith=0, inverse=False, order=0, center=None):
        ...
        offset = center - self.rotation.apply(center, inverse=inverse)

        # Check for invalid offsets
        if not np.isfinite(offset).all():
            raise ValueError(f"Invalid offset: {offset}")
        if np.any(np.abs(offset) > 10 * np.max(array.shape)):
            raise ValueError(f"Offset too large: {offset}")

        # Apply transform per channel or directly
        if array.ndim == 4:
            tarray = np.stack([
                scipy.ndimage.affine_transform(
                    array[i],
                    ...
                    order=order,
                    mode="constant",
                    cval=padwith[i] if hasattr(padwith, '__getitem__') else padwith,
                    offset=offset
                ) for i in range(array.shape[0])
            ], axis=0)
        else:
